...
import { 
  ArrowLeft, Server, MapPin, Shield, Globe, Copy, Check, Cpu, HardDrive, Zap, Network,
  ArrowUpRight, ArrowDownLeft, ScanEye, Clock, Activity
} from 'lucide-react';
...
export const NodeDetail = ({ item, onBack, isDark, onViewOnMap }) => {
  ...
  const colors = isDark 
    ? { bg: 'bg-[#161616]', card: 'bg-[#262626]', border: 'border-[#393939]', text: 'text-white', subText: 'text-[#8d8d8d]', accent: 'text-[#0F62FE]', success: 'text-[#24A148]', progressBg: 'bg-[#393939]' }
    : { bg: 'bg-[#f4f4f4]', card: 'bg-white', border: 'border-gray-200', text: 'text-gray-900', subText: 'text-gray-500', accent: 'text-blue-600', success: 'text-green-600', progressBg: 'bg-gray-100' };
  ...
  const getStatusColor = (status) => {
    ...
    if (isDark) {
      return isOnline ? 'text-green-300 bg-green-900/40' : 'text-red-300 bg-red-900/40';
    } else {
      return isOnline ? 'text-green-800 bg-green-100' : 'text-red-800 bg-red-100';
    }
  };
  ...
  return (
    <div className="animate-fade-in flex flex-col h-full w-full mx-auto pb-12">
      ...
      <div className="mb-6">
        <button 
          onClick={onBack} 
          className={`flex items-center gap-2 px-4 py-2 text-xs font-normal rounded-lg transition-all border ${isDark ? 'border-blue-500 text-blue-400 hover:bg-blue-600 hover:text-white hover:border-blue-600' : 'border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white hover:border-blue-600'}`}
        >
          <ArrowLeft size={14} />
          Back to Explorer
        </button>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
           ...
           <div>
              <div className="flex flex-wrap items-center gap-3 mb-2">
                <h2 className={`text-2xl font-light font-mono ${colors.text}`}>{item.fullAddress}</h2>
                <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider ${getStatusColor(item.status)}`}>{item.status}</span>
                <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-wider flex items-center gap-1 ${isPrivate ? (isDark ? 'text-purple-300 bg-purple-900/40' : 'text-purple-800 bg-purple-100') : (isDark ? 'text-blue-300 bg-blue-900/40' : 'text-blue-800 bg-blue-100')}`}>
              ...
              <div className={`text-xs ${colors.subText} font-mono flex flex-wrap items-center gap-4`}>
                <div className={`flex items-center gap-2 group cursor-pointer transition-colors ${isDark ? 'hover:text-white' : 'hover:text-black'}`} onClick={handleCopyId} title="Click to copy Node ID">
      ...
      <div className="mb-8 border-b border-[#393939]">
        <div className="flex gap-8">
          {['metrics', 'configuration'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`pb-3 text-sm font-medium border-b-2 transition-colors capitalize ${activeTab === tab ? 'border-[#0F62FE] text-[#0F62FE]' : 'border-transparent opacity-60 hover:opacity-100 hover:text-gray-400'} ${isDark ? 'text-white' : 'text-gray-900'}`}>{tab}</button>
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
            ...
            <div className={`${colors.card} border ${colors.border} p-6 rounded-lg flex flex-col`}>
                ...
                <div className="flex-1 flex flex-col justify-center">
                    <div className="flex justify-between items-end mb-2">
                        <span className={`text-3xl font-light ${colors.text}`}>{formatBytes(storageTotal)}</span>
                        <span className={`text-xs font-mono ${colors.subText}`}>{storagePercent.toFixed(1)}% Used</span>
                    </div>

                    <div className={`w-full h-2 rounded-sm overflow-hidden ${colors.progressBg}`}>
                        <div className={`h-full transition-all duration-1000 ease-out ${storagePercent > 90 ? 'bg-red-500' : 'bg-[#0F62FE]'}`} style={{ width: `${Math.max(storagePercent, 1)}%` }} />
                    </div>
            ...
            {showCpu && (
               <div className={`${colors.card} border ${colors.border} p-6 rounded-lg flex flex-col`}>
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                      <div className="flex justify-between items-end mb-2">
                          <span className={`text-3xl font-light ${colors.text}`}>{cpuPercent.toFixed(1)}%</span>
                          <span className={`text-xs font-mono ${colors.subText}`}>Load Average</span>
                      </div>

                      <div className={`w-full h-2 rounded-sm overflow-hidden ${colors.progressBg}`}>
                          <div className={`h-full transition-all duration-1000 ease-out ${cpuPercent > 80 ? 'bg-red-500' : 'bg-[#0F62FE]'}`} style={{ width: `${Math.min(cpuPercent, 100)}%` }} />
                      </div>
            ...
            {showRam && (
               <div className={`${colors.card} border ${colors.border} p-6 rounded-lg flex flex-col`}>
                  ...
                  <div className="flex-1 flex flex-col justify-center">
                      <div className="flex justify-between items-end mb-2">
                          <span className={`text-3xl font-light ${colors.text}`}>{formatBytes(ramUsed)}</span>
                          <span className={`text-xs font-mono ${colors.subText}`}>{ramPercent.toFixed(1)}%</span>
                      </div>

                      <div className={`w-full h-2 rounded-sm overflow-hidden ${colors.progressBg}`}>
                          <div className={`h-full transition-all duration-1000 ease-out ${ramPercent > 90 ? 'bg-red-500' : 'bg-[#24A148]'}`} style={{ width: `${Math.max(ramPercent, 1)}%` }} />
                      </div>
            ...
            <div className={`${colors.card} border ${colors.border} p-6 rounded-lg relative overflow-hidden`}>
                <div className="flex items-center gap-2 mb-4 relative z-10">
                    ...
                    {!isPrivate && (
                        <button onClick={() => onViewOnMap(item)} className={`ml-auto flex items-center gap-1.5 text-[10px] font-medium tracking-wide transition-all uppercase ${isDark ? 'text-gray-500 hover:text-blue-400' : 'text-gray-400 hover:text-blue-600'}`}>
                           <ScanEye size={14} /> View on Map
                        </button>
                    )}
                </div>
            ...
            <div className={`md:col-span-2 ${colors.card} border ${colors.border} p-6 rounded-lg`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded border ${colors.border} bg-opacity-50`}>
                        ...
                        <div className={`text-2xl font-light ${colors.text} flex items-center gap-2`}>{formatNumber(packetsSent)}<ArrowUpRight size={16} className="text-blue-500 opacity-70"/></div>
                    </div>
                    <div className={`p-4 rounded border ${colors.border} bg-opacity-50`}>
                        <div className={`text-xs ${colors.subText} uppercase tracking-wider mb-1`}>Packets Rcvd</div>
                        <div className={`text-2xl font-light ${colors.text} flex items-center gap-2`}>{formatNumber(packetsReceived)}<ArrowDownLeft size={16} className="text-green-500 opacity-70"/></div>
                    </div>
                </div>
                <div className={`mt-4 pt-3 border-t border-dashed ${isDark ? 'border-gray-800' : 'border-gray-200'} flex justify-between items-center`}>
                    ...
                    <div className="flex items-center gap-2"><Activity size={14} className={colors.success} /><span className={`font-mono text-sm ${colors.text}`}>Active</span></div>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
           ...
           <div className={`${colors.card} border ${colors.border} p-6 rounded-lg`}>
              ...
              <div className="space-y-4 text-sm">
                  <div className="flex justify-between py-2 border-b border-gray-800"><span className={colors.subText}>Node Type</span><span className={`font-mono ${colors.text}`}>{nodeTypeLabel}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-800"><span className={colors.subText}>Client Version</span><span className={`font-mono ${colors.text}`}>{item.version}</span></div>
                  <div className="flex justify-between py-2 border-b border-gray-800"><span className={colors.subText}>Protocol</span><span className={`font-mono ${colors.text}`}>v0.8 (Reinheim)</span></div>
                  {item.rpc_port && (<div className="flex justify-between py-2 border-b border-gray-800"><span className={colors.subText}>RPC Port</span><span className={`font-mono ${colors.text}`}>{item.rpc_port}</span></div>)}
              </div>
           </div>

           <div className={`${colors.card} border ${colors.border} p-6 rounded-lg`}>
              <div className="flex items-center gap-2 mb-6"><Zap size={18} className={colors.accent} /><h3 className={`text-sm font-semibold uppercase tracking-wider ${colors.text}`}>Runtime Status</h3></div>
              <div className="space-y-4 text-sm">
                   <div className="flex justify-between py-2 border-b border-gray-800"><span className={colors.subText}>Uptime</span><span className={`font-mono ${colors.text}`}>{formatUptime(item.uptime_seconds)}</span></div>
