...
export const Dashboard = ({ isDark, onNavigate, mapFocus }) => {
  ...
  const { stats, mapNodes, history } = useNetworkData();
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="mt-8" ref={mapSectionRef}>
        ...
        <div className={`w-full h-[500px] border rounded-lg overflow-hidden relative ${isDark ? 'bg-[#161616] border-[#393939]' : 'bg-white border-gray-200'}`}>
           {/* THE FIX: We pass 'mapNodes' into the 'nodes' prop. The map component won't know the difference. */}
           <WorldMap nodes={mapNodes} isDark={isDark} focusLocation={mapFocus} />
        </div>
      </div>
    </div>
  );
