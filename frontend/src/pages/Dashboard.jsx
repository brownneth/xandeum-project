import React from 'react'; // Removed useState, we don't need local state for theme anymore
...
// FIXED: Now accepts props (isDark, onNavigate) from App.jsx
export const Dashboard = ({ isDark, onNavigate }) => { 
  const { nodes, mapNodes, stats, history, loading } = useNetworkData();

  // Theme colors to ensure Dashboard matches the Layout
  const theme = isDark ? {
    cardBg: 'bg-[#161616]',
  ...
  if (loading && nodes.length === 0) {
    return (
      <div className={`flex items-center justify-center min-h-[50vh] ${theme.textColor}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard 
          ...
          color="blue"
          isDark={isDark} // Pass theme to cards
        />
        <StatCard 
          icon={Activity}
